...
<template>
  <div class="pos-view" v-if="pos.session">
    ...
    <!-- Global Store Toast Notification (Right-side) -->
    <Transition name="toast">
      <div v-if="pos.toast" class="pos-toast" :class="`pos-toast--${pos.toast.type || 'success'}`">
        <svg v-if="pos.toast.type === 'success'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="18" height="18">
          <polyline points="20 6 9 17 4 12"/>
        </svg>
        ...
        <div class="pos-toast__content">
          <span class="pos-toast__title">{{ pos.toast.title }}</span>
          <span class="pos-toast__name">{{ pos.toast.message }}</span>
        </div>
      </div>
    </Transition>
